...
依 [test_cases.md](test_cases.md) 的 16 條 TC 反推所需的 Page Object 與成員。
...
對應 TC：IN-04、BT-02、SO-01〜SO-03、PF-01〜PF-03

| 成員 | 類型 | 用途 | 服務的 TC |
|------|------|------|-----------|
| `SEARCH_INPUT` | locator | 結果頁頂部搜尋框（驗關鍵字保留） | IN-04 |
| `PRICE_SORT` | locator | 「價格」排序鈕 | SO-01, SO-02 |
| `RATING_SORT` | locator | 「評價」排序鈕 | SO-03 |
...
| `sort_by_rating()` | method | 點評價排序 | SO-03 |
...
對應 TC：SO-01〜SO-03、PF-01〜PF-03

| 成員 | 類型 | 用途 | 服務的 TC |
|------|------|------|-----------|
| `PRICE` | locator | 卡片價格 | SO-01, SO-02, PF-* |
| `RATING_STAR` | locator | 星等 | SO-03 |
| `REVIEW_COUNT` | locator | 評價數量 | SO-03 |
| `AD_BADGE` | locator | 廣告標記（判斷是否為廣告卡） | SO-*, PF-*（過濾用） |
| `price()` | method | 價格數值（去 `$` 與逗號轉 int） | SO-01, SO-02, PF-* |
| `rating()` | method | 星等數值（float） | SO-03 |
| `review_count()` | method | 評價數量（int） | SO-03 |
